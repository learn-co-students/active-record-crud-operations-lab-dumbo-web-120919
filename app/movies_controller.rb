...
def can_be_created_in_a_block(args = {title: "Home Alone", release_date: 1990})
  ...
  Movie.create do |m|
    m.title = args[:title] #movie-title assigned value of title argument
...
def can_find_the_first_item_from_the_database_using_id
  Movie.find_by(id:1) #use find_by to pull specific info from table?
end
...
def can_update_multiple_items_at_once
  ...
  Movie.all.each{|movie|  #<<< loop through all films to change them together
